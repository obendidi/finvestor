...
class YFBarsRequestParams(BaseModel):
    ...
    start: tp.Union[None, int, datetime] = Field(alias="period1")
    period: tp.Optional[ValidPeriod] = Field(alias="range")
    interval: AutoValidInterval
    end: tp.Union[None, int, datetime] = Field(alias="period2")
...
if __name__ == "__main__":
    start = datetime.now(tz=pytz.UTC) - timedelta(days=5)
    params = YFBarsRequestParams(interval="auto", period="1d")
    print(params)
    params.interval = "1m"
    print(params)
    params.interval = "2m"
    print(params.dict(by_alias=True, exclude_none=True))
